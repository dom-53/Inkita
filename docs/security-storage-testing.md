...
Steps to confirm tokens/API key live only in encrypted prefs:
...
3) Ensure plain DataStore no longer holds sensitive keys:
   - `adb shell run-as net.dom53.inkita strings files/datastore/inkita_prefs.preferences_pb | Select-String "token|api_key"` → should be empty output.
...
Notes:
- Migration wipes legacy keys even if they were blank, then writes tokens/API key into `secure_prefs`.
